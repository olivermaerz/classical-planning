...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        for effectA in actionA.effects: 
            if ~effectA in actionB.effects: # actionB effects negate effectA
                return True
        return False
    ...
    def _interference(self, actionA, actionB):
        ...
        for effect in actionA.effects:
            if ~effect in actionB.preconditions: # actionA effect negates actionB precondition 
                return True
        for effect in actionB.effects:
            if ~effect in actionA.preconditions: # the other way around
                return True
        return False
        ...
        for preconditionA in actionA.preconditions:
            for preconditionB in actionB.preconditions:
                if self.parent_layer.is_mutex(preconditionA, preconditionB): 
                    # preconditionA and preconditionB are mutex
                    return True
        return False
...
class LiteralLayer(BaseLiteralLayer):
    ...
    def _inconsistent_support(self, literalA, literalB):
        ...
        # every way to achieve both literals is mutex with each other 
        return True


    def _negation(self, literalA, literalB):
        """ Return True if two literals are negations of each other """
        return literalA == ~literalB # '~' is negation of the variable
...
class PlanningGraph:
    ...
    def h_levelsum(self):
        ...
        remaining_goals = set(self.goal) # set of goals we are looking for
        total = 0
        level = 0
        
        while remaining_goals:
            found = [] # list of goals found in level
            layer = self.literal_layers[level] 
            for goal in remaining_goals:
                if goal in layer: # we reached the goal
                    total += level
                    found.append(goal)
            remaining_goals = remaining_goals - set(found) 

            if remaining_goals: 
                self._extend() # expand graph 
                level += 1 
        return total
    ...
    def h_setlevel(self):
        ...
        while True:
            layer = self.literal_layers[level]
            all_goals_found = True # resetting at each level
                ...
                for goalA in self.goal:
                    for goalB in self.goal:
                        if layer.is_mutex(goalA, goalB):
                            # goalA and goalB is mutex in the layer
